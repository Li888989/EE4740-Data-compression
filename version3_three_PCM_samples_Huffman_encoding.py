...
samples_1 = audio_samples[::3]  
samples_2 = audio_samples[1::3]  
samples_3 = audio_samples[2::3]  

paired_samples = list(zip(samples_1, samples_2, samples_3))

symbol_counts = Counter(paired_samples)

symbol_freq_df = pd.DataFrame(symbol_counts.items(), columns=["Symbol Trigram", "Frequency"]).sort_values(by="Frequency", ascending=False)
...
encoded_data = "".join(huffman_codes[trigram] for trigram in paired_samples)
...
decoded_samples = np.array([val for trigram in decoded_pairs for val in trigram], dtype=np.int16)
